...
func TestLoginRefusesEverythingItShould(t *testing.T) {
	...
	for name, creds := range map[string][2]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			if got := decodeBody[map[string]string](t, rec)["error"]; got != "invalid_credentials" {
...
func TestInvitationSetsAPasswordExactlyOnce(t *testing.T) {
	...
	if got := decodeBody[map[string]string](t, rec)["error"]; got != "weak_password" {
	...
	who := decodeBody[userDTO](t, f.do(t, http.MethodGet, "/api/v1/auth/session", nil, cookie))
...
func TestRoleChangeTakesEffectOnTheNextRequest(t *testing.T) {
	...
	conflict := decodeBody[map[string]any](t, rec)
